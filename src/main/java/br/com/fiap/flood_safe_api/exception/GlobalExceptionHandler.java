...
public class GlobalExceptionHandler {
        ...
        public RestErrorResponse handleResourceNotFoundException(ResourceNotFoundException ex,
        ...
        // Manipula exceções de validação de argumentos (@Valid), retornando um
        // detalhamento dos campos inválidos (400 Bad Request).
        ...
        //Tratamento de exceção gerada por valores inválidos
        @ExceptionHandler(HttpMessageNotReadableException.class)
        @ResponseStatus(HttpStatus.BAD_REQUEST)
        public RestErrorResponse handleHttpMessageNotReadableException(HttpMessageNotReadableException ex,
                        HttpServletRequest request) {
                log.warn("Erro de desserialização na requisição: {}", ex.getMessage());

                String detailedMessage = "O corpo da requisição é inválido.";
                ...
                if (ex.getCause() instanceof InvalidFormatException ifx) {
                        if (ifx.getTargetType() != null && ifx.getTargetType().isEnum()) {
                                ...
                                detailedMessage = String.format("O valor '%s' não é válido. Valores aceitos: %s",
                                                ifx.getValue(),
                                                enumValues);
                        }
                }
        ...
        // Manipulador genérico para qualquer outra exceção não tratada (500 Internal
        // Server Error).
